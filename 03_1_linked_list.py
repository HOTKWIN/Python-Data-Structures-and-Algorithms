...
class Node(object):
    def __init__(self,value=None,next=None):    #这里默认root节点都是None
        self.value = value
        self.next = next
...
def test_linked_list():
    ...
    assert l1.remove(0) == 1
    assert l1.remove(10) == -1
    assert l1.remove(2) == 1
    assert len(l1) == 2
    assert list(l1) == [1,3]
    assert l1.find(0) == -1

    l1.appendleft(0)
    assert list(l1) == [0,1,3]
    assert len(l1) == 3

    headvalue = l1.popleft()
    assert headvalue == 0
    assert len(l1) == 2
    assert list(l1) == [1,3]
...
def test_linked_list_reverse():
    ...
    for i in range(n):
        l1.append(i)
    l1.reverse()
    assert list(l1) == list(reversed(range(n)))
